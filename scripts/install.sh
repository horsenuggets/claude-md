...
# Get script directory (where claude-md is cloned)
...
configure_startup_hook() {
    ...
    if [[ -f "$settings_file" ]]; then
        ...
        if grep -q "claude-md/CLAUDE.md" "$settings_file" 2>/dev/null; then
        ...
        if command -v jq &> /dev/null; then
            local hook_config='{"hooks":{"SessionStart":[{"hooks":[{"type":"command","command":"cat ~/git/claude-md/CLAUDE.md"}]}]}}'
