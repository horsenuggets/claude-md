#!/bin/bash
# Update claude-md submodules across all repos in ~/git
...
SUBMODULE_PATH="${SUBMODULE_PATH:-Submodules/claude-md}"

echo -e "${BLUE}Update claude-md Submodules${NC}"
echo "=========================================="
...
print_help() {
    ...
    echo "  SUBMODULE_PATH  Path to submodule within repos (default: Submodules/claude-md)"
}
...
find_repos_with_submodule() {
    ...
    for repo_dir in "$GIT_DIR"/*/; do
        ...
        # Skip if this is claude-md itself
        [[ "$(basename "$repo_dir")" == "claude-md" ]] && continue

        # Check if .gitmodules exists and contains our submodule
        local gitmodules="$repo_dir/.gitmodules"
        if [[ -f "$gitmodules" ]] && grep -q "claude-md" "$gitmodules" 2>/dev/null; then
            repos+=("$repo_dir")
        fi
    done
...
update_repo_submodule() {
    ...
    if [[ "$old_commit" != "$new_commit" ]]; then
        ...
        if [[ "$COMMIT_CHANGES" == "true" ]]; then
            ...
            git commit -m "Update claude-md submodule" 2>/dev/null || {
...
main() {
    ...
    if [[ ${#repos[@]} -eq 0 ]]; then
        echo "No repos found with claude-md submodule"
        exit 0
    fi
