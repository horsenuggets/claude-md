...
param(
    ...
    [string]$SubmodulePath = "Submodules/claude-md"
)
...
if ($Help) {
    ...
    Write-Host "  -SubmodulePath Path to submodule within repos (default: Submodules/claude-md)"
...
Write-Color "Update claude-md Submodules" "Cyan"
Write-Host "=========================================="
...
Get-ChildItem -Path $GitDir -Directory | ForEach-Object {
    ...
    if ($repoName -eq "claude-md") { return }
    ...
    if ((Test-Path $gitModules) -and ((Get-Content $gitModules -Raw) -match "claude-md")) {
        $repos += $repoPath
    }
}

if ($repos.Count -eq 0) {
    Write-Host "No repos found with claude-md submodule"
    exit 0
}
    ...
    try {
        ...
        if ($oldCommit -ne $newCommit) {
            ...
            if ($Commit) {
                ...
                git commit -m "Update claude-md submodule" 2>$null
