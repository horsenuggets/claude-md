...
# Syncs the claude-md repository and updates all installations
...
update_submodules() {
    ...
    if [[ -x "$submodule_script" ]]; then
        ...
        # Find all repos with claude-md submodule
        for repo in "$git_dir"/*/; do
            [[ -d "$repo" ]] || continue
            [[ -d "$repo/.git" ]] || continue

            # Check if this repo has claude-md as a submodule
            if [[ -f "$repo/.gitmodules" ]] && grep -q "claude-md" "$repo/.gitmodules" 2>/dev/null; then
                echo "  Updating submodule in: $(basename "$repo")"
                (
                    cd "$repo"
                    git submodule update --remote --merge Submodules/claude-md 2>/dev/null || true
                )
                ((count++))
            fi
        done
        ...
        if [[ $count -eq 0 ]]; then
            echo "  No repos with claude-md submodule found"
        else
