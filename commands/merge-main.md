...
1. Check if the current directory is inside `~/git/worktrees/`. If so, the repo name is the first directory under `worktrees/` (e.g., `~/git/worktrees/openclaw/tokyo` means the source repo is `openclaw`).
